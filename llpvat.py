...
def kl(model, optimizer, train_loader, alpha, consistency, scheduler, num_epochs, test_loader, device, logger):
    for epoch in range(num_epochs):
        logger.info(f"Epoch-{epoch}")
        logger.info(f"        lr: {optimizer.param_groups[0]['lr']}")
        llpvat_train_by_bag(model, optimizer, train_loader, epoch, alpha, consistency, device, scheduler, logger)
        if test_loader is not None:
            acc, test_error = test_model(model, test_loader, loss_f_test, device)
            logger.info(f"        accuracy = {100 * acc}%, test_error = {test_error}")
